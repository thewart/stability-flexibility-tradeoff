...
#plots for legends
# ggplot(switch_RT, aes(x = switch_perc, y = mean_RT, fill = switchType)) +
#   stat_summary(
#     fun.data = data_summary,
#     position = position_dodge(0.8), 
#     aes(shape = switchType)) +
#   scale_shape_manual(name = "Task Sequence", values = c(15, 17), labels = c("Repeat", "Switch")) +
#   theme(legend.title = element_text(face = "bold"))
# 
# ggplot(congruency_RT, aes(x = switch_perc, y = mean_RT, fill = stimCongruency)) +
#   stat_summary(
#     fun.data = data_summary,
#     position = position_dodge(0.8), 
#     aes(shape = factor(stimCongruency)), size = sum_size) +
#   scale_shape_manual(name = "switchType", values = c(1, 18)) +
...
ggplot(gl_RT, aes(x = taskCue, y = mean_RT, fill = taskCue)) +
  geom_jitter() +
  geom_boxplot(alpha = 0.5, outlier.colour = NA) +
  theme(legend.position = "none",
        axis.title.x = element_blank()) +
  ggtitle("Global vs Local RT") +
  scale_x_discrete(labels = c("Global","Local"))

gl_switchtype_RT <- df_trimmed %>% 
  group_by(subject, taskCue, switchType) %>% 
  summarise(mean_RT = mean(RT,na.rm = TRUE))

gl_switchcost <- gl_switchtype_RT %>%
  group_by(subject, taskCue) %>% 
  summarise(switch_cost = diff(mean_RT))

ggplot(gl_switchcost, aes(x = taskCue, y = switch_cost, fill = taskCue)) +
  geom_jitter() +
  geom_boxplot(alpha = 0.5, outlier.colour = NA) +
  theme(legend.position = "none",
        axis.title.x = element_blank()) +
  ggtitle("Global vs Local Switch Cost") +
  scale_x_discrete(labels = c("Global","Local"))

#pc effect and ps effect for correlation analysis
...
write.csv(effects, 'GL_effects.csv')
